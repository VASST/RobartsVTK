...
HistogramHolderDefault::HistogramHolderDefault(transferFunctionDefinitionWidget* parent, vtkCuda2DTransferFunction* f):
...
HistogramHolderDefault::~HistogramHolderDefault(){
}


void HistogramHolderDefault::setAutoUpdate(bool au){
  autoUpdate = au;
}

void HistogramHolderDefault::giveHistogramDimensions(float maxI1,float minI1, float maxI2, float minI2){
  histogram = true;
  maxIntensity1 = maxI1;
  minIntensity1 = minI1;
  maxIntensity2 = maxI2;
...
void HistogramHolderDefault::setZoomSquare(float maxI1,float minI1, float maxI2, float minI2){
  hasZoomSquare = true;
...
void HistogramHolderDefault::setObject(vtkCudaFunctionPolygon* object){
  this->object = object;
}

void HistogramHolderDefault::setSize(unsigned int s1, unsigned int s2){
  QWidget::setMaximumSize(s1,s2);
  QWidget::setMinimumSize(s1,s2);
  size1 = s1;
  size2 = s2;
}


void HistogramHolderDefault::keyPressEvent(QKeyEvent* e){
  if(e->key() == Qt::Key::Key_Shift) shiftHeld = true;
  if(e->key() == Qt::Key::Key_Control) ctrlHeld = true;
}

void HistogramHolderDefault::keyReleaseEvent(QKeyEvent* e){
  if(e->key() == Qt::Key::Key_Shift) shiftHeld = false;
  if(e->key() == Qt::Key::Key_Control) ctrlHeld = false;
}

void HistogramHolderDefault::mousePressEvent(QMouseEvent* e){
  ...
  float intensity1 = (float) e->x() / size1 * (maxIntensity1-minIntensity1) + minIntensity1;
  float intensity2 = (float) e->y() / size2 * (maxIntensity2-minIntensity2) + minIntensity2;
  ...
  if(e->buttons() == Qt::RightButton && object->GetNumVertices() > 3){
    ...
    unsigned int intensity1Location = size1 * (object->GetVertexIntensity(vertexInUse) - minIntensity1) / (maxIntensity1-minIntensity1);
    unsigned int intensity2Location = size2 * (object->GetVertexGradient(vertexInUse) - minIntensity2) / (maxIntensity2-minIntensity2);
  ...
  if(e->buttons() == Qt::LeftButton){
    ...
    for(int i = 0; i < object->GetNumVertices(); i++){
      ...
      float pixDistX = size1 * (intensity1-pointX) / (maxIntensity1- minIntensity1);
      float pixDistY = size2 * (intensity2-pointY) / (maxIntensity2- minIntensity2);
...
void HistogramHolderDefault::mouseReleaseEvent(QMouseEvent* e){
  //if we do not have a histogram or object, this has no meaning
  if(!histogram) return;
...
void HistogramHolderDefault::mouseMoveEvent(QMouseEvent* e){
...
void HistogramHolderDefault::paintEvent( QPaintEvent * e){
  if(!histogram) return;
  ...
  while(true){
    ...
    polygon.clear();
    for(unsigned int i = 0; i < numVertices; i++){
      unsigned int intensityLocation = size1 * (paintedObject->GetVertexIntensity(i) - minIntensity1) / (maxIntensity1-minIntensity1);
      unsigned int gradientLocation = size2 * (paintedObject->GetVertexGradient(i) - minIntensity2) / (maxIntensity2-minIntensity2);
  ...
  if(hasZoomSquare){
    ...
    colour.setBlue(255);
    painter.setPen(colour);
    painter.drawRect( size1 * (zoomMinIntensity1 - minIntensity1) / (maxIntensity1-minIntensity1),
      size2 * (zoomMinIntensity2 - minIntensity2) / (maxIntensity2-minIntensity2),
      size1 * (zoomMaxIntensity1 - zoomMinIntensity1) / (maxIntensity1-minIntensity1),
      size2 * (zoomMaxIntensity2 - zoomMinIntensity2) / (maxIntensity2-minIntensity2));
  }
}

void HistogramHolderDefault::visualizeAllObjects(bool b){
  visAll = b;
}