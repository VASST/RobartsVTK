...
transferFunctionDefinitionWidget::transferFunctionDefinitionWidget( transferFunctionWindowWidget* p, vtkCuda2DTransferFunction* f, int s1, int s2 ) :
...
void transferFunctionDefinitionWidget::setStandardWidgets( vtkRenderWindow* w, vtkRenderer* r, vtkCudaDualImageVolumeMapper* c){
	window = w;
	renderer = r;
	mapper = c;
}

void transferFunctionDefinitionWidget::setHistoSize(int s1, int s2){
	this->HISTOSIZE1 = s1;
	this->HISTOSIZE2 = s2;
}
...
unsigned int transferFunctionDefinitionWidget::getHistoSize(int i){
	if(i == 1)
		return HISTOSIZE1;
	if(i == 2)
		return HISTOSIZE2;
...
void transferFunctionDefinitionWidget::SetMax(double val, int scalar){
	if(scalar >= 0 && scalar < scalars)
		Max[scalar] = (Max[scalar] < val) ? val: Max[scalar];
}

void transferFunctionDefinitionWidget::SetMin(double val, int scalar){
	if(scalar >= 0 && scalar < scalars)
...
void transferFunctionDefinitionWidget::selectImage(vtkImageData*d){
	...
	scalars = (sJump-1)/2;
	if(this->Max) delete this->Max;
	if(this->Min) delete this->Min;
...
void transferFunctionDefinitionWidget::setRed(int index){
	redChosen = index;
	computeHistogram();
	SignalRed(index);
}

void transferFunctionDefinitionWidget::setGreen(int index){
	greenChosen = index;
	computeHistogram();
	SignalGreen(index);
}

void transferFunctionDefinitionWidget::setBlue(int index){
	blueChosen = index;
	computeHistogram();
	SignalBlue(index);
}

char* transferFunctionDefinitionWidget::getHistogram(vtkImageData* map, float retIntensity1Low, float retIntensity1High, float retIntensity2Low, float retIntensity2High, bool setSize){
