...
qTransferFunctionDefinitionWidget::qTransferFunctionDefinitionWidget( qTransferFunctionWindowWidget* p,vtkCuda2DTransferFunction* f )
  : QWidget(p)
  , function(f)
{
  parent = p;
...
void qTransferFunctionDefinitionWidget::setStandardWidgets( vtkRenderWindow* w, vtkRenderer* r, vtkCuda2DVolumeMapper* c )
{
  window = w;
  renderer = r;
  mapper = c;
...
unsigned int qTransferFunctionDefinitionWidget::getHistoSize()
{
  return HISTOSIZE;
}
...
public:
  ...
  void run()
  {
    ...
    {
      for(unsigned int x = 0; x < dimX; x++,index++)
      {
...
char* qTransferFunctionDefinitionWidget::getHistogram(vtkImageData* image, float& retIntensityLow, float& retIntensityHigh, float& retGradientLow, float& retGradientHigh, bool setSize)
{
