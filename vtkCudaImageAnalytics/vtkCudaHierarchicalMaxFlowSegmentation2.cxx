...
void vtkCudaHierarchicalMaxFlowSegmentation2::Reinitialize(int withData = 0){
	//no long-term data stored and no helper classes, so no body for this method
}

void vtkCudaHierarchicalMaxFlowSegmentation2::Deinitialize(int withData = 0){
	//no long-term data stored and no helper classes, so no body for this method
}
//------------------------------------------------------------
...
void vtkCudaHierarchicalMaxFlowSegmentation2::PropogateLabels( vtkIdType currNode ){
...
void vtkCudaHierarchicalMaxFlowSegmentation2::SolveMaxFlow( vtkIdType currNode, int* TimeStep ){
...
void vtkCudaHierarchicalMaxFlowSegmentation2::UpdateLabel( vtkIdType node, int* TimeStep ){
...
void vtkCudaHierarchicalMaxFlowSegmentation2::GetGPUBuffers(){
...
void vtkCudaHierarchicalMaxFlowSegmentation2::ReturnBufferGPU2CPU(float* CPUBuffer, float* GPUBuffer){
...
void vtkCudaHierarchicalMaxFlowSegmentation2::MoveBufferCPU2GPU(float* CPUBuffer, float* GPUBuffer){
	if( NoCopyBack.find(CPUBuffer) != NoCopyBack.end() ) return;
	CUDA_CopyBufferToGPU( GPUBuffer, CPUBuffer, VolumeSize, GetStream());
	NumMemCpies++;
}

void vtkCudaHierarchicalMaxFlowSegmentation2::BuildStackUpToPriority( int priority ){
...
void vtkCudaHierarchicalMaxFlowSegmentation2::AddToStack( float* CPUBuffer ){
...
void vtkCudaHierarchicalMaxFlowSegmentation2::RemoveFromStack( float* CPUBuffer ){
	int neededPriority = CPU2PriorityMap.find(CPUBuffer)->second;
	std::list< std::list< float* > >::iterator stackIterator = PriorityStacks.begin();
	std::list< int >::iterator priorityIterator = Priority.begin();
	for( ; *priorityIterator != neededPriority; stackIterator++, priorityIterator++);
...
class vtkCudaHierarchicalMaxFlowSegmentation2::CircListNode {
public:
	CircListNode* Prev;
	CircListNode* Next;
...
void vtkCudaHierarchicalMaxFlowSegmentation2::ClearBufferOrdering( vtkIdType currNode ){
	//push down to leaves
	int NumKids = this->Hierarchy->GetNumberOfChildren(currNode);
...
void vtkCudaHierarchicalMaxFlowSegmentation2::UpdateBufferOrderingAt( float* buffer, int reference ){
...
void vtkCudaHierarchicalMaxFlowSegmentation2::SimulateIterationForBufferOrdering( vtkIdType currNode, int* reference ){
	//get number of kids
	int NumKids = this->Hierarchy->GetNumberOfChildren(currNode);
...
void vtkCudaHierarchicalMaxFlowSegmentation2::SimulateIterationForBufferOrderingUpdateLabelStep( vtkIdType currNode, int* reference ){
	int NumKids = this->Hierarchy->GetNumberOfChildren(currNode);
...
void vtkCudaHierarchicalMaxFlowSegmentation2::DeallocatePrioritySet(){
...
void vtkCudaHierarchicalMaxFlowSegmentation2::GetGPUBuffersV2(int reference){
