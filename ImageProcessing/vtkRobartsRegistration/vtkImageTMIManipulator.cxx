/*=========================================================================
  ...
  Module:    $RCSfile: vtkImageTMIManipulator.cxx,v $
  Language:  C++
...
//----------------------------------------------------------------------------
vtkImageTMIManipulator* vtkImageTMIManipulator::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkImageTMIManipulator");
  if(ret)
  {
    return (vtkImageTMIManipulator*)ret;
  }
  // If the factory was unable to create the object, then create it here.
  return new vtkImageTMIManipulator;
}

//----------------------------------------------------------------------------
vtkImageTMIManipulator::vtkImageTMIManipulator()
{
  for (int i = 0; i <= 5; i++)
  {
    this->Extent[i] = 0;
...
//----------------------------------------------------------------------------
void vtkImageTMIManipulator::SetInput1(vtkImageData *input)
{
...
//----------------------------------------------------------------------------
void vtkImageTMIManipulator::SetInput2(vtkImageData *input)
{
  this->inData[1] = input;
}

//----------------------------------------------------------------------------
vtkImageData *vtkImageTMIManipulator::GetInput1()
{
  return this->inData[0];
}

//----------------------------------------------------------------------------
vtkImageData *vtkImageTMIManipulator::GetInput2()
{
  return this->inData[1];
}

//----------------------------------------------------------------------------
void vtkImageTMIManipulator::SetBinNumber(int numS, int numT)
{
...
//----------------------------------------------------------------------------
void vtkImageTMIManipulator::SetMaxIntensities(int maxS, int maxT)
{
...
//----------------------------------------------------------------------------
template <class T>
void vtkImageTMIManipulatorEntropyT(vtkImageTMIManipulator *self,
                                    T  *inPtr, int inc2[2], double count)
{
  ...
  self->entropyT = ( 1.0 / (1.0 - self->qValue) * (entropyT / pow(count,self->qValue) - 1) );
}

//----------------------------------------------------------------------------
void vtkImageTMIManipulator::SetExtent(int ext[6])
{
  void *inPtr;
...
//----------------------------------------------------------------------------
void vtkImageTMIManipulator::SetTranslation(double tran[3])
{
  double f[3];
...
//----------------------------------------------------------------------------
template <class T>
void vtkImageTMIManipulatorExecute(vtkImageTMIManipulator *self,
                                   T  *in1Ptr, T *in2Ptr,
                                   ...
                                   int loc000[3], int loc111[3], double count)
{
  ...
  // Calculate entropies and the TMI
  entropyS = ( 1.0 / (1.0 - self->qValue) * (entropyS / pow(count,self->qValue) - 1) );
  entropyST = ( 1.0 / (1.0 - self->qValue) * (entropyST / pow(count,self->qValue) - 1) );

  self->Result = entropyS+self->entropyT+(1-self->qValue)*entropyS*self->entropyT-entropyST;
}

//----------------------------------------------------------------------------
double vtkImageTMIManipulator::GetResult()
{
  // Check inputs.
...
//----------------------------------------------------------------------------
void vtkImageTMIManipulator::PrintSelf(ostream& os, vtkIndent indent)
{
