/*=========================================================================
  ...
  Module:    $RCSfile: vtkImageSMIManipulator2.cxx,v $
  Language:  C++
...
//----------------------------------------------------------------------------
vtkImageSMIManipulator2* vtkImageSMIManipulator2::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkImageSMIManipulator2");
  if(ret)
  {
    return (vtkImageSMIManipulator2*)ret;
  }
  // If the factory was unable to create the object, then create it here.
  return new vtkImageSMIManipulator2;
}

//----------------------------------------------------------------------------
vtkImageSMIManipulator2::vtkImageSMIManipulator2()
{
  for (int i = 0; i <= 5; i++)
  {
    this->Extent[i] = 0;
...
//----------------------------------------------------------------------------
void vtkImageSMIManipulator2::SetInput1(vtkImageData *input)
{
...
//----------------------------------------------------------------------------
void vtkImageSMIManipulator2::SetInput2(vtkImageData *input)
{
  this->inData[1] = input;
}

//----------------------------------------------------------------------------
void vtkImageSMIManipulator2::SetMask(vtkImageData *mask)
{
  this->inData[2] = mask;
}

//----------------------------------------------------------------------------
vtkImageData *vtkImageSMIManipulator2::GetInput1()
{
  return this->inData[0];
}

//----------------------------------------------------------------------------
vtkImageData *vtkImageSMIManipulator2::GetInput2()
{
  return this->inData[1];
}

//----------------------------------------------------------------------------
vtkImageData *vtkImageSMIManipulator2::GetMask()
{
  return this->inData[2];
}

//----------------------------------------------------------------------------
void vtkImageSMIManipulator2::SetBinNumber(int numS, int numT)
{
...
//----------------------------------------------------------------------------
void vtkImageSMIManipulator2::SetMaxIntensities(int maxS, int maxT)
{
...
//----------------------------------------------------------------------------
template <class T>
void vtkImageSMIManipulator2EntropyT(vtkImageSMIManipulator2 *self,
                                     T  *inPtr1, T  *inPtr2, int inc2[2])
{
  short b;
  double temp, entropyT = 0, count = 0;
  ...
  for (int idZ = self->Extent[4]; idZ <= self->Extent[5]; idZ++)
  {
    for (int idY = self->Extent[2]; idY <= self->Extent[3]; idY++)
    {
      for (int idX = self->Extent[0]; idX <= self->Extent[1]; idX++)
      {
        ...
        {
          count++;
          b = vtkResliceRound(*inPtr1/double(self->BinWidth[1]));
          if (b < 0)
          {
            b = 0;
          }
  ...
  {
    cout << "ERROR: No data to work with\n";
    exit(0);
  }
}

//----------------------------------------------------------------------------
void vtkImageSMIManipulator2::SetExtent(int ext[6])
{
  //void *inPtr;
  ...
  this->inPtr[2] = this->inData[2]->GetScalarPointerForExtent(ext);
...
//----------------------------------------------------------------------------
void vtkImageSMIManipulator2::SetTranslation(double tran[3])
{
  double f[3];
...
//----------------------------------------------------------------------------
template <class T>
void vtkImageSMIManipulator2Execute(vtkImageSMIManipulator2 *self,
                                    T  *in1Ptr, T *in2Ptr, T *in3Ptr,
                                    vtkIdType inc[3], int inc2[2], int inExt[6],
                                    int loc000[3], int loc111[3])
{
  double V000, V100, V010, V110, V001, V101, V011, V111, Vxyz;
  double temp, entropyS = 0, entropyST = 0, count = 0;
  short a, b;
  int i, j;
       ...
       (self->Extent[5] + loc111[2] > inExt[5]) )
  {
    if (self->Metric == 0)
    {
      self->Result = 0.5;  // Lowest NMI
    }
    if (self->Metric == 1)
    {
      self->Result = 0.0;  // Lowest MI
    }
    if (self->Metric == 2)
    {
      self->Result = 1.0;  // Highest ECR
    }
    return;
  }
...
//----------------------------------------------------------------------------
double vtkImageSMIManipulator2::GetResult()
{
  // Check inputs.
  ...
  {
    vtkTemplateMacro(vtkImageSMIManipulator2Execute(this,
                     (VTK_TT *)(this->inPtr[0]), (VTK_TT *)(this->inPtr[1]), (VTK_TT *)(this->inPtr[2]),
...
//----------------------------------------------------------------------------
void vtkImageSMIManipulator2::PrintSelf(ostream& os, vtkIndent indent)
{
