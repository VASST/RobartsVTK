/*=========================================================================
  ...
  Module:    $RCSfile: vtkImageNCCManipulator.cxx,v $
  Language:  C++
...
//----------------------------------------------------------------------------
vtkImageNCCManipulator* vtkImageNCCManipulator::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkImageNCCManipulator");
  if(ret)
    {
    return (vtkImageNCCManipulator*)ret;
    }
  // If the factory was unable to create the object, then create it here.
  return new vtkImageNCCManipulator;
}

//----------------------------------------------------------------------------
vtkImageNCCManipulator::vtkImageNCCManipulator()
{
  for (int i = 0; i <= 5; i++) { this->Extent[i] = 0; }
}

//----------------------------------------------------------------------------
void vtkImageNCCManipulator::SetInput1(vtkImageData *input)
{
...
//----------------------------------------------------------------------------
void vtkImageNCCManipulator::SetInput2(vtkImageData *input)
{
  this->inData[1] = input;
}

//----------------------------------------------------------------------------
vtkImageData *vtkImageNCCManipulator::GetInput1()
{
  return this->inData[0];
}

//----------------------------------------------------------------------------
vtkImageData *vtkImageNCCManipulator::GetInput2()
{
  return this->inData[1];
}

//----------------------------------------------------------------------------
void vtkImageNCCManipulator::SetExtent(int ext[6])
{
...
//----------------------------------------------------------------------------
void vtkImageNCCManipulator::SetTranslation(double tran[3])
{
  double f[3];
				  ...
				  int loc000[3], int loc111[3])
{
      ...
      for (int idZ = self->Extent[4]; idZ <= self->Extent[5]; idZ++)
	{
	  for (int idY = self->Extent[2]; idY <= self->Extent[3]; idY++)
	      ...
	      for (int idX = self->Extent[0]; idX <= self->Extent[1]; idX++)
		{
		  if (*in1Ptr || *in2Ptr)
		    {
		      sumST += (double)*in1Ptr * (double)*in2Ptr;
		      sumS += (double)*in1Ptr * (double)*in1Ptr;
		      sumT += (double)*in2Ptr * (double)*in2Ptr;
		    }
		  in1Ptr++;
		  in2Ptr++;
		}
  ...
  self->Result = sumST / (sqrt(sumS) * sqrt(sumT));

}

//----------------------------------------------------------------------------
double vtkImageNCCManipulator::GetResult()
{
...
//----------------------------------------------------------------------------
void vtkImageNCCManipulator::PrintSelf(ostream& os, vtkIndent indent)
{
