...
texture<float, 2, cudaReadModeElementType> inExLogic_texture_2D;
...
cudaArray* inExLogicTransferArray2D = 0;
...
__device__ void CUDAkernel_CastRays2D(float3& rayStart,
	...
	while( maxSteps > 0 ){
		...
		float inEx = tex2D(inExLogic_texture_2D, tempIndex, gradMag);
		...
		if(alpha > 0.0f && tempIndex >= 0.0f && tempIndex <= 1.0f && gradMag >= 0.0f && gradMag <= 1.0f){
			...
			if(!backStep){
				...
				if( !( excludeStart >= maxSteps && excludeEnd <= maxSteps ) && inEx > 0.5 ) break;
	...
	outputVal.w = (outputVal.w < 1.0f) ? 1.0f : 0.0f;
...
void CUDA_vtkCuda2DInExLogicVolumeMapper_renderAlgo_loadTextures(const cuda2DTransferFunctionInformation& transInfo,
	...
	cudaMallocArray( &inExLogicTransferArray2D, &channelDesc, transInfo.functionSize, transInfo.functionSize);
	...
	//define the texture mapping for the blue component after copying information from host to device array
	cudaMemcpyToArray(inExLogicTransferArray2D, 0, 0, inExTF, size*transInfo.functionSize, cudaMemcpyHostToDevice);
	inExLogic_texture_2D.normalized = true;
	inExLogic_texture_2D.filterMode = cudaFilterModePoint;
	inExLogic_texture_2D.addressMode[0] = cudaAddressModeClamp;
	inExLogic_texture_2D.addressMode[1] = cudaAddressModeClamp;
	cudaBindTextureToArray(inExLogic_texture_2D, inExLogicTransferArray2D, channelDesc);
