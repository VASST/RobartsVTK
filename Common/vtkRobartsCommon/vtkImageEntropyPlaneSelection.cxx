...
{
  if( slice < Extent[0] || slice > Extent[1] || !EntropyInX )
  {
    return -1.0;
  }
...
{
  if( slice < Extent[2] || slice > Extent[3] || !EntropyInY )
  {
    return -1.0;
  }
...
{
  if( slice < Extent[4] || slice > Extent[5] || !EntropyInZ )
  {
    return -1.0;
  }
    ...
    for(int y = 0; y < Extent[3]-Extent[2]+1; y++)
      for(int x = 0; x < Extent[1]-Extent[0]+1; x++, idx++)
      {
        ...
        for(int i = 0; i < this->GetNumberOfInputConnections(0); i++)
        {
          ...
          if( accumulator == 0.0 )
          {
            entropy = log( (double) this->GetNumberOfInputConnections(0) );
          }
