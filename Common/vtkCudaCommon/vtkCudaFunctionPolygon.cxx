...
const size_t vtkCudaFunctionPolygon::GetNumVertices()
{
  return contour.size();
}
...
void vtkCudaFunctionPolygon::AddVertex( float intensity, float gradient, size_t index )
{
...
void vtkCudaFunctionPolygon::ModifyVertex( float intensity, float gradient, size_t index )
{
...
const float vtkCudaFunctionPolygon::GetVertexIntensity( size_t index )
{
  if( index >= contour.size() )
  {
    return -1.0f;
...
const float vtkCudaFunctionPolygon::GetVertexGradient( size_t index )
{
  if( index >= contour.size() )
  {
    return -1.0f;
...
void vtkCudaFunctionPolygon::RemoveVertex( size_t index )
{
  //if we choose a vertex not in the polygon, exit
  if( index >= contour.size() )
  {
    ...
    int logUsed)
{
  ...
  if( lowIntensityIndex < 0 )
  {
    lowIntensityIndex = 0;
  }
  if( lowIntensityIndex >= IntensitySize )
  {
    lowIntensityIndex = IntensitySize-1;
  }
  int highIntensityIndex = (double)(IntensitySize-1) * (maxI - IntensityLow) / (IntensityHigh-IntensityLow);
  if( highIntensityIndex < 0 )
  {
    highIntensityIndex = 0;
  }
  if( highIntensityIndex >= IntensitySize )
  {
    highIntensityIndex = IntensitySize-1;
  }
  int lowGradientIndex = (double)(GradientSize-1) * (minG - GradientLow) / (GradientHigh-GradientLow);
  if( lowGradientIndex < 0 )
  {
    lowGradientIndex = 0;
  }
  if( lowGradientIndex >= GradientSize )
  {
    lowGradientIndex = GradientSize-1;
  }
  int highGradientIndex = (double)(GradientSize-1) * (maxG - GradientLow) / (GradientHigh-GradientLow);
  if( highGradientIndex < 0 )
  {
    highGradientIndex = 0;
  }
  if( highGradientIndex >= GradientSize )
  {
    highGradientIndex = GradientSize-1;
  }
  ...
  for(int g = lowGradientIndex; g < highGradientIndex; g++)
  {
    ...
    for(int i = lowIntensityIndex; i < highIntensityIndex; i++)
    {
      ...
      if( inside )
      {
        ...
        if(aTable)
        {
          aTable[tableIndex] = this->Ambient;
        }
        if(dTable)
        {
          dTable[tableIndex] = this->Diffuse;
        }
        if(sTable)
        {
          sTable[tableIndex] = this->Specular;
        }
        if(pTable)
        {
          pTable[tableIndex] = this->SpecularPower;
        }
      }
    }
  }
