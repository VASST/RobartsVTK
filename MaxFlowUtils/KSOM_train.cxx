...
int main(int argc, char** argv){
  ...
  while(!ScheduleStream.eof()){
    double Iteration; double mA, vA, wA, mW, vW, wW;
  ...
  if(LabelFileList.empty()){
    ...
    while(!DataFileStream.eof()){
      std::string DataFileName; std::getline(DataFileStream,DataFileName);
    ...
    while(!DataFileStream.eof()){
      ...
      std::string LabelFileName; std::getline(LabelFileStream,LabelFileName);
